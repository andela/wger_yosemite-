...
class GymListView(LoginRequiredMixin, PermissionRequiredMixin,
    ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['global_gym_config'] = \
            GlobalGymConfig.objects.all().first()
        return context
...
def gym_new_user_info_export(request):
    ...
    new_user = get_object_or_404(User, pk=request.session['gym.user'
                                 ]['user_pk'])
    ...
    writer.writerow([_('Username'), _('First name'), _('Last name'),
                    _('Gym'), _('Password')])
    writer.writerow([new_username, new_user.first_name,
                    new_user.last_name, new_user.userprofile.gym.name,
                    password])
    ...
    filename = \
        'User-data-{t.year}-{t.month:02d}-{t.day:02d}-{user}.csv'.format(t=today,
            user=new_username)
...
def gym_permissions_user_edit(request, user_pk):
    ...
    if not user.has_perm('gym.manage_gyms') \
        and not user.has_perm('gym.manage_gym'):
        return HttpResponseForbidden()

    if user.has_perm('gym.manage_gym') and user.userprofile.gym \
        != member.userprofile.gym:
        return HttpResponseForbidden()
...
class GymAddUserView(WgerFormMixin, LoginRequiredMixin,
    ...
    def dispatch(
        ...
        **kwargs
        ...
        return super(GymAddUserView, self).dispatch(request, *args,
                **kwargs)
        ...
        **kwargs
        ...
        if request.user.has_perm('gym.manage_gym') \
            ...
            if request.user.userprofile.gym_id != int(self.kwargs['pk'
                ...
                return HttpResponseForbidden()
        return super(GymUpdateView, self).dispatch(request, *args,
                **kwargs)
