...
def gym_permissions_user_edit(request, user_pk):
    ...
    if not user.has_perm('gym.manage_gyms') and not user.\
            has_perm('gym.manage_gym'):
        return HttpResponseForbidden()

    if user.has_perm('gym.manage_gym') and \
            user.userprofile.gym != member.userprofile.gym:
        return HttpResponseForbidden()
...
class GymAddUserView(WgerFormMixin,
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if request.user.has_perm('gym.manage_gym') \
                ...
                and request.user.userprofile.gym_id != int(self.
                                                           kwargs['gym_pk']):
            return HttpResponseForbidden()
    ...
    def get_form(self):
        ...
        '''
        return self.\
            form_class(available_roles=get_permission_list(self.
                                                           request.user),
