...
class Command(BaseCommand):
    ...
    help = ('Download exercise images from wger.de and update\
     the local database\n'
            '\n'
            'ATTENTION: The script will download the images from \
            the server and add them\n'
            '           to your local exercises. The exercises are\
             identified by\n'
            '           their UUID field, if you manually edited or\
             changed it\n'
    ...
    def handle(self, **options):
        ...
        if not settings.MEDIA_ROOT:
            raise ImproperlyConfigured('Please set MEDIA_ROOT\
        ...
        headers = {'User-agent': default_user_agent('wger/{} + \
        requests'.format(get_version()))}
        ...
        for exercise_json in result['results']:
            ...
            self.stdout.write(u"*** Processing {0}\
             (ID: {1}, UUID:              {2})".format(exercise_name,
            ...
            try:
                ...
                self.stdout.write('Remote exercise not found \
                in local DB, skipping...')
                continue
            ...
            if images['count']:
                ...
                for image_json in images['results']:
                    ...
                    try:
                        ...
                        self.stdout.write('    --> Image already \
                        present locally, skipping...')
                        continue
                    except ExerciseImage.DoesNotExist:
                        self.stdout.write('   --> Image not found \
                        in local DB, creating now...')
                        image = ExerciseImage()
                        image.pk = image_id
                ...
                self.stdout.write(' No images for\
                 this exercise, nothing to do')
