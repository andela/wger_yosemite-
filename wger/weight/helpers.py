...
def group_log_entries(user, year, month, day=None):
    ...
    # There can be workout sessions without any associated log entries, so it
    # is not enough so simply iterate through the logs
    if day:
...
def process_log_entries(logs):
    ...
    for entry in logs:
        ...
        if entry.weight != max_weight[entry.date][entry.reps]:
            continue
        if (entry.date, entry.reps, entry.weight) in entry_list[entry.reps][
                'seen']:
            continue
...
def get_last_entries(user, amount=5):
    ...
    last_entries = WeightEntry.objects.filter(user=user).order_by(
        '-date')[:5]
