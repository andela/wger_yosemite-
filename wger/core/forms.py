...
class UserEmailForm(forms.ModelForm):
    ...
    def clean_email(self):
        ...
        email, otherwise the uniqueness check will because it will find
        one user (the current one) using the same email. Only
        when the user changes it, do we want to check that nobody
        else has that email
        '''
...
class FeedbackRegisteredForm(forms.Form):
    ...
    contact = forms.CharField(max_length=50,
                              min_length=10,
                              label=_('Contact'),
                              help_text=_('Some way of answering you' +
                                          ' (email, etc.)'),
                              required=False)
