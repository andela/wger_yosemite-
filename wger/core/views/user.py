    ...
    if user_pk:
        ...
        if not request.user.has_perm('gym.manage_gyms') \
                and (not request.user.has_perm('gym.manage_gym')
                     or request.user.userprofile.gym_id != user.
                     userprofile.gym_id
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request,
                             _('Account "{0}" was successfully deleted').
...
def trainer_login(request, user_pk):
    ...
    # Note: it seems we have to manually set the authentication backend
    # here
    if own:
...
def registration(request):
    ...
    if request.user.is_authenticated() and not request.user. \
            userprofile.is_temporary:
...
class UserActivateView(LoginRequiredMixin,
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if (request.user.has_perm('gym.manage_gym') or
            request.user.has_perm('gym.gym_trainer')) \
                and edit_user.userprofile.gym_id != request.user.\
                userprofile.gym_id:
            return HttpResponseForbidden()

        return super(UserActivateView, self).\
            dispatch(request, *args, **kwargs)
...
class UserEditView(WgerFormMixin,
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if user.has_perm('gym.manage_gym') \
                ...
                and user.userprofile.gym != \
                self.get_object().userprofile.gym:
            return HttpResponseForbidden()

        return super(UserEditView, self).dispatch(request, *args, **kwargs)

    def get_success_url(self):
        return reverse('core:user:overview',
...
class UserDetailView(LoginRequiredMixin,
    ...
    def get_context_data(self, **kwargs):
        ...
        context['weight_entries'] =\
            WeightEntry.objects.filter(user=self.object)\
            .order_by('-date')[:5]
        context['nutrition_plans'] = \
            NutritionPlan.objects.filter(user=self.object)\
            .order_by('-creation_date')[:5]
        context['session'] = WorkoutSession.objects.filter(
            user=self.object).order_by('-date')[:10]
        context['admin_notes'] = \
            AdminUserNote.objects.filter(member=self.object)[
            :5]
