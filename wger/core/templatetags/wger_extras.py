...
def get_current_settings(exercise, set_id):
    ...
    to function in the template, and we are only interested on the settings
...
def pagination(paginator, page):
    ...
    # For very long lists (e.g. the English ingredient with more
    # than 8000 items) we muck around here to remove the pages not
    # inmediately 'around' the current one, otherwise we end up with a
    # useless block with 300 pages.
...
def auto_link_css(flavour='full', css=''):
    '''
    Adds the appropriate classes to a sidebar link depending on the
    site version
...
class SpacelessNode(template.base.Node):
    ...
    def render(self, context):
        if settings.WGER_SETTINGS['REMOVE_WHITESPACE']:
            ...
            return strip_spaces_between_tags(self.
...
def is_checkbox(field):
    '''
    Tests if a field element is a checkbox, as it needs to be handled
    slightly different
...
def is_fileupload(field):
    '''
    Tests if a field element is a file upload, as it needs to be handled
    slightly different
