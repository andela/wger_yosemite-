...
def delete_testcase_add_methods(cls):
    ...
    for user in get_user_list(cls.user_fail):
        def test_unauthorized(self):
            """Tests if the eligible users are the only ones allowed"""
...
class WorkoutManagerDeleteTestCase(WorkoutManagerTestCase):
    ...
    and a logged out one. This assumes the delete action is only triggered
    ...
    def delete_object(self, fail=False):
        ...
        if fail:
            ...
            #       (e.g. /user/login instead of /en/user/login) so there
...
class WorkoutManagerEditTestCase(WorkoutManagerTestCase):
    ...
    fileupload = None
    '''
    If the form requires a file upload, specify the field name and the
    file path here in a list or tuple:
...
class WorkoutManagerAddTestCase(WorkoutManagerTestCase):
    ...
    def add_object(self, fail=False):
        ...
        # Enter the data
        count_before = self.object_class.objects.count()
        self.pk_before = self.object_class.objects.all().order_by('id').\
            last().pk
        ...
        if self.fileupload:
            ...
            response = self.client.post(get_reverse(self.url), self.data)
        count_after = self.object_class.objects.count()
        self.pk_after = self.object_class.objects.all().order_by('id').\
            last().pk
...
class WorkoutManagerAccessTestCase(WorkoutManagerTestCase):
    ...
    def access(self, fail=True):
        ...
        if fail:
            ...
            # TODO: the redirection page might not have a language prefix
            #       (e.g. /user/login instead of /en/user/login) so there is
            #       an additional
            #     redirect if response.status_code == 302:
            #     # The page we are redirected to doesn't trigger an error
            #     response = self.client.get(response['Location'])
            #     self.assertEqual(response.status_code, 200)
