    ...
    def get_current_workout(self, user):
        ...
        try:
            ...
            if schedule.schedulestep_set.count():
                ...
                # in the past and is not a loop, we won't use it. Doing it
                # like this is kind of wrong, but lets us continue to the
                # correct place
...
class Day(models.Model):
    ...
    def get_first_day_id(self):
        '''
        Return the PK of the first day of the week, this is used
        in the template
    ...
    def get_canonical_representation(self):
        ...
        for set_obj in self.set_set.select_related():
            ...
            for exercise in set_obj.exercises.select_related():
                ...
                setting_list = reps_smart_text(setting_tmp, set_obj)
                weight_units = reps_smart_text(setting_tmp, set_obj)
                repetition_units = reps_smart_text(setting_tmp, set_obj)
                setting_text = reps_smart_text(setting_tmp, set_obj)
                weight_list = reps_smart_text(setting_tmp, set_obj)
                reps_list = reps_smart_text(setting_tmp, set_obj)
            ...
            # If it's a superset, check that all exercises have the
            # same repetitions. If not, just take the smallest number
            # and drop the rest, because otherwise
...
class Setting(models.Model):
    ...
    Note that since adding the unit field, the name
    is no longer correct, but is
...
class WorkoutSession(models.Model):
    ...
    IMPRESSION = ((IMPRESSION_BAD, _("Bad")), (IMPRESSION_NEUTRAL,
                                               _('Neutral')), (IMPRESSION_GOOD,
                                                               _('Good')), )
