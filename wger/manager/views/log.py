...
def add(request, pk):
    ...
    # We need several lists here because we need to assign
    # specificform to each
    # exercise: the entries for weight
    # and repetitionshave no indicator to which
    ...
    if request.method == 'POST':
        ...
        # that the form expects a value for
        # the exercise which is not present in
        ...
        if dateform.is_valid() and session_form.is_valid(
            ...
            # Save the Workout Session only if there
            # is not already one for this date
        ...
        # Depending on whether there is already
        # a workout session for today, update
        # the current one or create a new one
        ...
        for day_list in self.object.canonical_representation['day_list']:
            ...
            for set_list in day_list['set_list']:
                ...
                for exercise_list in set_list['exercise_list']:
                    ...
                    #       reason (bug
                    #       in django? DB problems?) when adding the filter
                    #       there, the
                    #       execution time explodes. The weight unit filter
                    #       works as
                    #       expected. Also, adding the unit IDs to the exclude
                    #       list
                    #       also has the disadvantage that if new ones are
                    #       added in a
