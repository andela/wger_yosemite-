...
def export_pdf(request, id, uidb64=None, token=None):
    ...
    * http://www.reportlab.com/apis/reportlab/dev/platypus.html
    ...
    for meal in plan.meal_set.select_related():
        ...
        if not meal.time:
            p = Paragraph(u'<para align="center"><strong>{nr} ' +
                          '{meal_nr}</strong></para>'
        ...
        for item in meal.mealitem_set.select_related():
            ...
            p = Paragraph(
                u'<para>{0}</para>'.format(item.ingredient.name),
                styleSheet["Normal"])
    ...
    # There is nothing to output
        ...
        t = Paragraph(_('<i>This is an empty plan, what did you' +
                        ' expect on the PDF?</i>'),
                      styleSheet["Normal"])
    ...
    data.append([Paragraph(u'<para align="center">{0}</para>'.
                           ...
                           styleSheet["Bold"])])
    data.append([Paragraph(_('Macronutrients'),
                           styleSheet["Normal"]),
